...
async function queryTasks() {
  const result = await session.run(
    `MATCH (t:Task {assignee: 'plato'}) RETURN t.id as id, t.title as title, t.status as status, t.subject as subject, t.createdAt as createdAt ORDER BY t.createdAt DESC`
  );
