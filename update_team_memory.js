// Update AYBKK shared memory with latest bot schema
...
async function main() {
  ...
  const result = await session.run(`
    ...
    SET tm.updatedAt = datetime(),
        ...
        tm.studentNode = '{studentId (UUID), name, firstName, lastName, email, phone, nationality, pgId, active, createdAt, updatedAt}',
        ...
        tm.notes = 'Student tracking bot with voting on asana progression. Teachers assess independently via Telegram bot. Each vote saved separately per teacher.'
    RETURN tm.id as id
  `);
